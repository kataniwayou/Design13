...
/// </summary>
public class ProcessingChainEntityManager : AbstractManagerService<AbstractProcessingChainEntity, string>
{
    private readonly Dictionary<string, AbstractProcessingChainEntity> _processingChains = new();
    private readonly object _lock = new();

    /// <summary>
    /// Gets the manager type that this manager implements.
    /// </summary>
    public override string ManagerType => "ProcessingChainEntityManager";

    /// <summary>
    /// Creates a new instance of the ProcessingChainEntityManager class.
    /// </summary>
    ...
    public ProcessingChainEntityManager(string id, string name, string description, string version)
    ...
    public override Task<IEnumerable<AbstractProcessingChainEntity>> GetAllEntitiesAsync()
    {
        lock (_lock)
        {
            return Task.FromResult<IEnumerable<AbstractProcessingChainEntity>>(_processingChains.Values.ToList());
        }
    }
    ...
    public override Task<AbstractProcessingChainEntity?> GetEntityAsync(string id)
    {
        lock (_lock)
        {
            _processingChains.TryGetValue(id, out var entity);
    ...
    public override Task<ResourceStatus> GetEntityStatusAsync(string id)
    {
        lock (_lock)
        {
            if (_processingChains.TryGetValue(id, out var entity))
            {
    ...
    public override Task<RegistrationResult> RegisterAsync(AbstractProcessingChainEntity entity)
    {
        if (entity == null)
        {
    ...
    public override Task<bool> UnregisterEntityAsync(string id)
    {
        lock (_lock)
        {
            return Task.FromResult(_processingChains.Remove(id));
        }
    }
    ...
    public override Task<bool> UpdateEntityStatusAsync(string id, ResourceStatus status)
    {
        lock (_lock)
        {
            if (_processingChains.TryGetValue(id, out _))
            {
    ...
    public override Task<Abstractions.Services.ValidationResult> ValidateAsync(AbstractProcessingChainEntity entity)
    {
        if (entity == null)
        {
