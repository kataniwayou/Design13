...
/// </summary>
public class DestinationEntityManager : AbstractManagerService<AbstractDestinationEntity, string>
{
    private readonly Dictionary<string, AbstractDestinationEntity> _destinations = new();
    private readonly object _lock = new();

    /// <summary>
    /// Gets the manager type that this manager implements.
    /// </summary>
    public override string ManagerType => "DestinationEntityManager";

    /// <summary>
    /// Creates a new instance of the DestinationEntityManager class.
    /// </summary>
    ...
    public DestinationEntityManager(string id, string name, string description, string version)
    ...
    public override Task<IEnumerable<AbstractDestinationEntity>> GetAllEntitiesAsync()
    {
        lock (_lock)
        {
            return Task.FromResult<IEnumerable<AbstractDestinationEntity>>(_destinations.Values.ToList());
        }
    }
    ...
    public override Task<AbstractDestinationEntity?> GetEntityAsync(string id)
    {
        lock (_lock)
        {
            _destinations.TryGetValue(id, out var entity);
    ...
    public override Task<ResourceStatus> GetEntityStatusAsync(string id)
    {
        lock (_lock)
        {
            if (_destinations.TryGetValue(id, out var entity))
            {
    ...
    public override Task<RegistrationResult> RegisterAsync(AbstractDestinationEntity entity)
    {
        if (entity == null)
        {
    ...
    public override Task<bool> UnregisterEntityAsync(string id)
    {
        lock (_lock)
        {
            return Task.FromResult(_destinations.Remove(id));
        }
    }
    ...
    public override Task<bool> UpdateEntityStatusAsync(string id, ResourceStatus status)
    {
        lock (_lock)
        {
            if (_destinations.TryGetValue(id, out _))
            {
    ...
    public override Task<Abstractions.Services.ValidationResult> ValidateAsync(AbstractDestinationEntity entity)
    {
        if (entity == null)
        {
