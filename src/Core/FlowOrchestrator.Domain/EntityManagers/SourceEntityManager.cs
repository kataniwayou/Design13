...
/// </summary>
public class SourceEntityManager : AbstractManagerService<AbstractSourceEntity, string>
{
    private readonly Dictionary<string, AbstractSourceEntity> _sources = new();
    private readonly object _lock = new();

    /// <summary>
    /// Gets the manager type that this manager implements.
    /// </summary>
    public override string ManagerType => "SourceEntityManager";

    /// <summary>
    /// Creates a new instance of the SourceEntityManager class.
    /// </summary>
    ...
    public SourceEntityManager(string id, string name, string description, string version)
    ...
    public override Task<IEnumerable<AbstractSourceEntity>> GetAllEntitiesAsync()
    {
        lock (_lock)
        {
            return Task.FromResult<IEnumerable<AbstractSourceEntity>>(_sources.Values.ToList());
        }
    }
    ...
    public override Task<AbstractSourceEntity?> GetEntityAsync(string id)
    {
        lock (_lock)
        {
            _sources.TryGetValue(id, out var entity);
    ...
    public override Task<ResourceStatus> GetEntityStatusAsync(string id)
    {
        lock (_lock)
        {
            if (_sources.TryGetValue(id, out var entity))
            {
    ...
    public override Task<RegistrationResult> RegisterAsync(AbstractSourceEntity entity)
    {
        if (entity == null)
        {
    ...
    public override Task<bool> UnregisterEntityAsync(string id)
    {
        lock (_lock)
        {
            return Task.FromResult(_sources.Remove(id));
        }
    }
    ...
    public override Task<bool> UpdateEntityStatusAsync(string id, ResourceStatus status)
    {
        lock (_lock)
        {
            if (_sources.TryGetValue(id, out _))
            {
    ...
    public override Task<Abstractions.Services.ValidationResult> ValidateAsync(AbstractSourceEntity entity)
    {
        if (entity == null)
        {
