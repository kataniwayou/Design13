...
/// </summary>
public class DestinationAssignmentEntityManager : AbstractManagerService<AbstractDestinationAssignmentEntity, string>
{
    private readonly Dictionary<string, AbstractDestinationAssignmentEntity> _destinationAssignments = new();
    private readonly object _lock = new();

    /// <summary>
    /// Gets the manager type that this manager implements.
    /// </summary>
    public override string ManagerType => "DestinationAssignmentEntityManager";

    /// <summary>
    /// Creates a new instance of the DestinationAssignmentEntityManager class.
    /// </summary>
    ...
    public DestinationAssignmentEntityManager(string id, string name, string description, string version)
    ...
    public override Task<IEnumerable<AbstractDestinationAssignmentEntity>> GetAllEntitiesAsync()
    {
        lock (_lock)
        {
            return Task.FromResult<IEnumerable<AbstractDestinationAssignmentEntity>>(_destinationAssignments.Values.ToList());
        }
    }
    ...
    public override Task<AbstractDestinationAssignmentEntity?> GetEntityAsync(string id)
    {
        lock (_lock)
        {
            _destinationAssignments.TryGetValue(id, out var entity);
    ...
    public override Task<ResourceStatus> GetEntityStatusAsync(string id)
    {
        lock (_lock)
        {
            if (_destinationAssignments.TryGetValue(id, out var entity))
            {
    ...
    public override Task<RegistrationResult> RegisterAsync(AbstractDestinationAssignmentEntity entity)
    {
        if (entity == null)
        {
    ...
    public override Task<bool> UnregisterEntityAsync(string id)
    {
        lock (_lock)
        {
            return Task.FromResult(_destinationAssignments.Remove(id));
        }
    }
    ...
    public override Task<bool> UpdateEntityStatusAsync(string id, ResourceStatus status)
    {
        lock (_lock)
        {
            if (_destinationAssignments.TryGetValue(id, out _))
            {
    ...
    public override Task<Abstractions.Services.ValidationResult> ValidateAsync(AbstractDestinationAssignmentEntity entity)
    {
        if (entity == null)
        {
