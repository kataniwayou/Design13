...
/// </summary>
public class ScheduledFlowEntityManager : AbstractManagerService<AbstractScheduledFlowEntity, string>
{
    private readonly Dictionary<string, AbstractScheduledFlowEntity> _scheduledFlows = new();
    private readonly object _lock = new();

    /// <summary>
    /// Gets the manager type that this manager implements.
    /// </summary>
    public override string ManagerType => "ScheduledFlowEntityManager";

    /// <summary>
    /// Creates a new instance of the ScheduledFlowEntityManager class.
    /// </summary>
    ...
    public ScheduledFlowEntityManager(string id, string name, string description, string version)
    ...
    public override Task<IEnumerable<AbstractScheduledFlowEntity>> GetAllEntitiesAsync()
    {
        lock (_lock)
        {
            return Task.FromResult<IEnumerable<AbstractScheduledFlowEntity>>(_scheduledFlows.Values.ToList());
        }
    }
    ...
    public override Task<AbstractScheduledFlowEntity?> GetEntityAsync(string id)
    {
        lock (_lock)
        {
            _scheduledFlows.TryGetValue(id, out var entity);
    ...
    public override Task<ResourceStatus> GetEntityStatusAsync(string id)
    {
        lock (_lock)
        {
            if (_scheduledFlows.TryGetValue(id, out var entity))
            {
                var status = new ResourceStatus
                {
                    ResourceId = id,
                    Status = entity.IsEnabled ? ServiceStatus.Running : ServiceStatus.Stopped,
    ...
    public override Task<RegistrationResult> RegisterAsync(AbstractScheduledFlowEntity entity)
    {
        if (entity == null)
        {
    ...
    public override Task<bool> UnregisterEntityAsync(string id)
    {
        lock (_lock)
        {
            return Task.FromResult(_scheduledFlows.Remove(id));
        }
    }
    ...
    public override Task<bool> UpdateEntityStatusAsync(string id, ResourceStatus status)
    {
        lock (_lock)
        {
            if (_scheduledFlows.TryGetValue(id, out var entity))
            {
                // Update the enabled status based on the service status
                entity.IsEnabled = status.Status == ServiceStatus.Running;
    ...
    public override Task<Abstractions.Services.ValidationResult> ValidateAsync(AbstractScheduledFlowEntity entity)
    {
        if (entity == null)
        {
